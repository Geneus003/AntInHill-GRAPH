def StartProc(matrix, n):
    ...
    def main_func(matrix, matrix_is, n, le_road):
        ...
        for q in range(len(max_cof_kor)):
            ...
            le, matrix_w, maska = main_func(matrix_w, matrix_is, n, now_road)
            # print(maska)

            if le <= min_road:
                min_road = le
                maskad.append([max_cof_kor[q][1], max_cof_kor[q][0]])
        ...
        if s == (-2 * n * n):
            return le, matrix_w, maskad

        return le_road, matrix, maskad

    le_road, matrix, maskad = main_func(matrix, matrix_is, n, le_road)
